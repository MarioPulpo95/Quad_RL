...
class NoTiltEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for k in range(5):
            ...
            self.get_Velocity()
            roll_ref, pitch_ref, yaw_ref = self.PositionController.VelocityController(action,self.vx, self.vy, self.yaw)
            self.get_RPY()
            self.get_Velocity()
    ...
    def get_reward(self,done):
        ...
        TILT = -np.linalg.norm(self.drone_angular)
        self.distance_to_goalinfo = {}
        terminated = False
...
if __name__ == '__main__':
    try:
        ...
        reg = register(id='Quad-v0', entry_point='quadv0:NoTiltEnv', max_episode_steps=LENGTH)
        start_train()
